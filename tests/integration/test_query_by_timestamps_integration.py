...
def test_cli_query_timestamps_command_log(tmp_path):
...
"""
    ...
    # Construct the path to the syslog_manager main.py file
    script_path = Path(__file__).resolve().parents[2] / "syslog_manager" / "main.py"

    # Run the command using subprocess for the .log format
    result = subprocess.run(
        [
            sys.executable, str(script_path), "query", "log", str(temp_file), "between", "14/06/2024", "15/06/2024"
        ],
...
def test_cli_query_timestamps_command_json(tmp_path):
    # Create a temporary syslog JSON file
    syslog_data = [
...
"""
    ...
    # Run the command using subprocess for the .json format
    result = subprocess.run(
        [
            sys.executable, str(script_path), "query", "json", str(temp_file), "between", "14/06/2024", "15/06/2024"
        ],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
...
def test_cli_query_timestamps_command_csv(tmp_path):
    # Create a temporary syslog CSV file
    syslog_data = """\
...
"""
    ...
    # Run the command using subprocess for the .csv format
    result = subprocess.run(
        [
            sys.executable, str(script_path), "query", "csv", str(temp_file), "between", "14/06/2024", "15/06/2024"
        ],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
