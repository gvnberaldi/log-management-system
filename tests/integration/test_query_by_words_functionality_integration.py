...
def test_cli_query_contains_words_command_log(tmp_path):
...
def test_cli_query_contains_words_command_json(tmp_path):
    # Create a temporary syslog file
    syslog_data = [
...
"""
    ...
    result = subprocess.run(
        [
            sys.executable, str(script_path), "query", "json", str(temp_file), "contains_words", "failure,Accepted,abnormally"
        ],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
...
def test_cli_query_contains_words_command_csv(tmp_path):
    syslog_data = """\
...
"""
    ...
    result = subprocess.run(
        [
            sys.executable, str(script_path), "query", "csv", str(temp_file), "contains_words", "failure,Accepted,abnormally"
        ],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
